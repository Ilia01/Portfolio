...
When building Aegis2FA as a learning project, I set a goal: **80%+ test coverage before calling it "done"**.

Not because tests guarantee correctness, but because writing tests forces you to think about edge cases. And in authentication, edge cases become security vulnerabilities.

Here's what I learned achieving 84% coverage:
- **Found 3 race conditions** before ever deploying
- **Caught a token reuse vulnerability** with a simple test
- **Prevented a DoS attack** by testing password length limits

More importantly, I learned how to structure tests for real systems. These testing patterns now guide how I write tests for production code at Andersen.
...
**Integration tests catch more bugs than unit tests**: 70% of bugs I found were caught by integration tests hitting real database and Redis.

**E2E tests are expensive but worth it for critical flows**: Don't test everything end-to-end. Test the flows that matter—login, 2FA setup, token refresh.

**Write tests before fixing bugs**: When you find a bug, write a test that reproduces it first. Then fix it. The test becomes documentation of the bug and insurance it doesn't come back.

**Coverage numbers don't tell the whole story**: 80% coverage with good tests beats 95% coverage with tests that don't test anything meaningful.

I'm now applying this testing approach to production systems at Andersen—focusing on integration tests for critical paths and accepting that not everything needs 100% coverage.
